...
// one user (the sender) to another (the recipient). Return an array of users that make up this route.

// There might be a few shortest delivery routes, all with the same length. For now, let's just
// return any shortest route.

// Your network information takes the form of an object where keys are usernames and values are
// arrays of other users nearby:
//
// const network = {
...
// Gotchas
// We can find the shortest route in O(N+M) time, where N is the number of users and M is the
// number of connections between them.
// It's easy to write code that can get caught in an infinite loop for some inputs! Does your
// code always finish running?
...
// Solution
// We treat the input user network as a graph in adjacency list format. Then we do a breadth-first
// search from the sender, stopping once we reach the recipient.
...
// after our breadth-first search reaches the end node, we use our object to backtrack from the
// recipient to the sender.

// To make sure our breadth-first search terminates, we're careful to avoid visiting any node twice.
// We could keep track of the nodes we've already seen in a set, but, to save space, we reuse the
// object we've already set up for recovering the path.
...
// Complexity
// Our solution has two main steps. First, we do a breadth-first search of the user network starting
// from the sender. Then, we use the results of our search to backtrack and find the shortest path.

// How much work is a breadth-first search?

// In the worst case, we'll go through the BFS loop once for every node in the graph, since we only
// ever add each node to nodesToVisit once (we check howWeReachedNodes to see if we've already
// added a node before). Each loop iteration involves a constant amount of work to dequeue the
// node and check if it's our end node. If we have nn nodes, then this portion of the loop is O(N).

// But there's more to each loop iteration: we also look at the current node's neighbors. Over all
// of the nodes in the graph, checking the neighbors is O(M)O(M), since it involves crossing each
// edge twice: once for each node at either end.
...
// What about backtracking to determine the shortest path? Handling each node in the path is O(1),
// and we could have at most N nodes in our shortest path. So, that's O(N) for building up the
// path. Then, it's another O(N) to reverse it. So, the total time complexity of our backtracking
// step is O(N).

// Putting these together, the time complexity of our entire algorithm is O(N+M).

// What about space complexity? The queue of nodes to visit, the mapping of nodes to previous nodes,
// and the final path ... they all store a constant amount of information per node. So, each data
// structure could take up to O(N) space if it stored information about all of our nodes.
